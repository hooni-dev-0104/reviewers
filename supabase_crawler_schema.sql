...
begin
  ...
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='security_rate_limits' and policyname='Service role only security rate limits'
  ) then
    create policy "Service role only security rate limits" on public.security_rate_limits for all using (false) with check (false);
  end if;
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='campaign_snapshots' and policyname='Service role only campaign snapshots'
  ) then
    create policy "Service role only campaign snapshots" on public.campaign_snapshots for all using (false) with check (false);
  end if;
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='crawl_jobs' and policyname='Service role only crawl jobs'
  ) then
    create policy "Service role only crawl jobs" on public.crawl_jobs for all using (false) with check (false);
  end if;
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='crawl_errors' and policyname='Service role only crawl errors'
  ) then
    create policy "Service role only crawl errors" on public.crawl_errors for all using (false) with check (false);
  end if;
  if not exists (
    select 1 from pg_policies where schemaname='public' and tablename='source_policies' and policyname='Service role only source policies'
  ) then
    create policy "Service role only source policies" on public.source_policies for all using (false) with check (false);
  end if;
  if not exists (
